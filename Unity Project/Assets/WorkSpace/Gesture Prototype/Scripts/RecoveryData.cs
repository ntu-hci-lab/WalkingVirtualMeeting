...
public class RecoveryData : MonoBehaviour
{
    ...
    private void recovery(PlayerData playerData)
    {
        ...
        for (int i = 0; i < playerData.presentation.Count; i++)
        {
            ...
            if (relative)
            {
                p2.transform.localScale = new Vector3(1.0f, 1.0f, 1.0f);
    ...
    private void applyData(WindowSetting w, GameObject g)
    {
        ...
        //g.transform.FindChildRecursive("RawImage").GetComponent<UnityEngine.UI.RawImage>().color = new Vector4(0f, 0.53f, 1f, w.transparency);
    }
