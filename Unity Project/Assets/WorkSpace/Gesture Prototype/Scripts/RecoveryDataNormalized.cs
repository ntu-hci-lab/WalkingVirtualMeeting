...
public class RecoveryDataNormalized : MonoBehaviour
{
    public bool export;
    ...
    private void recovery(PlayerData playerData)
    {
        ...
        for (int i = 0; i < playerData.presentation.Count; i++)
        {
            ...
            // normalizing
            p1.transform.parent = p2.transform;
            p2.transform.localEulerAngles = new Vector3(0f, 0f, 0f);
            ...
            //print(p2.transform.localPosition);
            ...
            if (export)
            {
                ...
                //WindowData w1 = new WindowData(vector2float(p1.transform.position), vector2float(p1.transform.eulerAngles), p1.transform.forward, p1.transform.GetChild(0).position, p1.transform.GetChild(1).position, p1.transform.GetChild(2).position, p1.transform.GetChild(3).position, playerData.participant[i].scale[0]);
                WindowData w1 = new WindowData(vector2float(p1.transform.localPosition), vector2float(p1.transform.localEulerAngles), p1.transform.forward, p1.transform.GetChild(0).position, p1.transform.GetChild(1).position, p1.transform.GetChild(2).position, p1.transform.GetChild(3).position, p1.transform.localScale[0]);
                w1.type = 1;
                w1.transparency = playerData.participant[i].transparency;
                exportData.addWindowData(p.name, w1);
                //print(p1.transform.localPosition);
                WindowData w2 = new WindowData(vector2float(p2.transform.localPosition), vector2float(p2.transform.localEulerAngles), p2.transform.forward, p2.transform.GetChild(0).position, p2.transform.GetChild(1).position, p2.transform.GetChild(2).position, p2.transform.GetChild(3).position, p2.transform.localScale[0]);
    ...
    private void applyData(WindowSetting w, GameObject g)
    {
        ...
        g.transform.FindChildRecursive("RawImage").GetComponent<UnityEngine.UI.RawImage>().color = new Vector4(0f, 0f, 0f, w.transparency);
    }
    private Vector3 float2vector(float[] f)
    {
        return new Vector3(f[0], f[1], f[2]);
    }
    private float[] vector2float(Vector3 v)
    {
